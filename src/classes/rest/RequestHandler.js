...
const Endpoints = require('./EndpointConstants')
...
class RequestHandler {
  constructor (client) {
    ...
    this.userAgent = `DiscordBot (https://github.com/thesharks/midgard, ${require('../../../package.json').version})`
    this.ratelimits = {}
  ...
  request (method, url, auth, body, file, _route, short) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const actualCall = (cb) => {
        ...
        try {
          if (auth) {
            headers.Authorization = `Bot ${this._client._token}`
          }
        ...
        req.once('response', (resp) => {
          ...
          if (this.latencyRef.lastTimeOffsetCheck < Date.now() - 5000) {
            ...
            if (this.latencyRef.timeOffset - this.latencyRef.latency >= 30000 && timeOffset - this.latencyRef.latency >= 30000) {
              this._client.emit('warn', new Error('Your clock is 30000ms behind Discord\'s server clock. Please check your connection and system time.'))
            }
